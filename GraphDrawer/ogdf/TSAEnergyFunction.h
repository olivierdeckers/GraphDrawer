...
#ifndef TSAENERGYFUNCTION_H
#define TSAENERGYFUNCTION_H
 ...
 */
class TSAEnergyFunction {
public:
    //! Initializes data dtructures to speed up later computations
    TSAEnergyFunction(const String &funcname, GraphAttributes &AG);
    virtual ~TSAEnergyFunction() { }
    ...
    double computeCandidateEnergy(Hashing<node, DPoint> &layoutChanges);
...
protected:
    ...
    Hashing<node, DPoint> *m_layoutChanges;
    //! returns the current position of vertex v
    DPoint currentPos(const node v) const {return DPoint(m_AG.x(v),m_AG.y(v));}

    DPoint newPos(const node v) const {return (m_layoutChanges != NULL && m_layoutChanges->member(v)) ? m_layoutChanges->lookup(v)->info() : currentPos(v); }
...
private:
    ...
    TSAEnergyFunction(const TSAEnergyFunction &e):m_G(e.m_G),m_name(e.m_name),m_AG(e.m_AG) { }
    //! the assignment operator is fake and can not be used.
    TSAEnergyFunction& operator=(const TSAEnergyFunction &e);
